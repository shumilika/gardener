...
const BlogCard: React.FC<BlogCardProps> = ({ image, title, slug, date }) => {
  return (
    <div className="h-min w-full flex flex-col items-center justify-center rounded-[15px]">
      <Link
        href={`/blog/${slug}`}
        className="group w-full h-full flex flex-col items-center justify-end gap-[30px]"
      >
        <div className="flex flex-col items-center relative w-full h-[329px]">
          <div className="absolute inset-0">
            ...
            />
            <div
              className="absolute inset-0 w-full h-full flex flex-col justify-center rounded-[15px]
            ...
            >
              <ArrowUpRightIcon
                className="size-20 text-white opacity-0 transition-all duration-500 ease-in-out
